  ...
  return (
    <div className="panel">
      <div className="inv-controls">
        <strong style={{ color: 'var(--gold-bright)' }}>{t('st.inv.title')}</strong>
        <span className="muted">{t('st.inv.owned', { n: ownedCount })}</span>
        <span className="spacer" />
        <input className="input" placeholder={t('st.search')} value={q} onChange={(e) => setQ(e.target.value)} />
        <select className="input" value={cat} onChange={(e) => setCat(e.target.value as Category | 'all')}>
          {CATS.map((c) => <option key={c} value={c}>{c === 'all' ? t('st.inv.allcats') : CATEGORY_LABEL[c as Category]}</option>)}
        </select>
        <select className="input" value={stat} onChange={(e) => setStat(e.target.value)}>
          <option value="all">{t('st.inv.allstats')}</option>
          {ALL_STATS.map((s) => <option key={s.key} value={s.key}>{s.label}</option>)}
        </select>
        <label className="row" style={{ gap: 4 }}>
          <input type="checkbox" checked={onlyOwned} onChange={(e) => setOnlyOwned(e.target.checked)} /> {t('st.inv.onlyowned')}
        </label>
      </div>
      ...
      <div style={{ maxHeight: '64vh', overflowY: 'auto' }}>
        <table className="inv-table">
          <thead>
            <tr>
              <th>Soul</th>
              <th>{t('st.inv.col.cat')}</th>
              <th>{t('st.inv.col.attr')}</th>
              <th>L1 / L2 / L3</th>
              <th>{t('st.inv.col.have')}</th>
            </tr>
          </thead>
          <tbody>
            {list.map((s) => {
                ...
                <tr key={s.id} className={owned ? 'owned' : ''}>
                  ...
                  <td>
                    <div className="lvl-btns">
                      <button className={`lvl-btn off ${owned === 0 ? 'active' : ''}`} onClick={() => setOwned(s.id, 0)} title={t('st.inv.donthave')}>—</button>
                      {[1, 2, 3].map((lv) => (
