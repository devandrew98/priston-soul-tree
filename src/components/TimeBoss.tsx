...
export function TimeBoss() {
  ...
  const [audioOn, setAudioOn] = useState(() => localStorage.getItem('tb-audio') === '1');
  ...
  const toggleAudio = () => {
    const nx = !audioOn;
    setAudioOn(nx);
    localStorage.setItem('tb-audio', nx ? '1' : '0');
    if (nx) {
      try {
  ...
  return (
    <div className="tb">
      ...
      {/* coming up (name + countdown only, no portraits — cleaner) */}
      {rest.length > 0 && (
        <section className="tb-section">
          ...
          <div className="tb-next-grid slim">
            {rest.map((ev, i) => (
              ...
              >
                <div className="tb-next-names slim">
