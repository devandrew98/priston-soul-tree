...
/** PvP nodes come in two in-game flavors: offensive (✚ icon) and defensive
 *  (shield icon, frame NODE-Com-PVP-DEF). Each accepts only matching PvP souls. */
export type PvpKind = 'atk' | 'def';
...
export interface TreeNode {
  ...
  pvpKind?: PvpKind; // only set when type === 'pvp'
}
...
export function nodeFrameSrc(type: NodeType, rarity: Rarity, pvpKind?: PvpKind): string {
  // The game ships a dedicated frame for the defensive PvP node (shield art).
  if (type === 'pvp' && pvpKind === 'def' && rarity === 'common') return '/fusion/NODE-Com-PVP-DEF.png';
...
interface RawNode {
  ...
  pvpKind?: PvpKind; // PVP nodes only: offensive or defensive flavor
}
...
const RAW_NODES: RawNode[] = [
  ...
  { id: 'cv_6', x: 3, y: 6, category: 'PVP', rarity: 'COMMON', pvpKind: 'atk' },
  ...
  { id: 'h2_3', x: 3, y: 7, category: 'PVP', rarity: 'RARE', pvpKind: 'atk' },
  ...
  { id: 'bv3_8', x: 3, y: 8, category: 'PVP', rarity: 'COMMON', pvpKind: 'def' },
  { id: 'bv3_9', x: 3, y: 9, category: 'PVP', rarity: 'LEGENDARY', pvpKind: 'atk' },
...
/** Stats that make a PvP soul OFFENSIVE; PvP souls without any are defensive. */
const PVP_OFFENSIVE_STATS = new Set(['attackPower', 'attackRating', 'critRate']);

/** Flavor of a PvP soul (null for non-PvP souls). The data splits cleanly:
 *  offensive = AP/AR/crit souls; defensive = defense/absorb/hp/evade/block. */
export function pvpSoulKind(soul: { category: Category; stats: { stat: string }[] }): PvpKind | null {
  if (soul.category !== 'pvp') return null;
  return soul.stats.some((st) => PVP_OFFENSIVE_STATS.has(st.stat)) ? 'atk' : 'def';
}

/**
 * Whether a soul (its category + rarity) may be placed on a node.
 * Rules (from the wiki + in-game tree):
 ...
 *  - PvP nodes have a flavor: the defensive PvP node (shield icon) takes only
 *    defensive PvP souls, the offensive ones only offensive PvP souls.
 */
export function acceptsSoul(
  ...
  soulRarity: Rarity,
  nodePvpKind?: PvpKind,
  soulPvpKind?: PvpKind | null,
): boolean {
  if (accepts === 'wildcard') {
    if (RARITY_ORDER[soulRarity] < RARITY_ORDER['rare']) return false;
  } else if (accepts !== soulCat) {
    return false;
  } else if (accepts === 'pvp' && nodePvpKind && soulPvpKind && nodePvpKind !== soulPvpKind) {
    return false;
  }
...
export function nodeAccepts(node: TreeNode, soulCat: Category, soulRarity: Rarity): boolean {
  return acceptsSoul(NODE_CATEGORY[node.type], node.rarity, soulCat, soulRarity, node.pvpKind);
}
