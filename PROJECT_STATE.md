...
10. **Sabores de PvP (atk/def) + correção do layout** — o print do jogo mostrou que o node `bv3_8` (penúltimo do ramo central de baixo) é **PvP DEFENSIVO** (ícone de escudo, moldura própria `NODE-Com-PVP-DEF.png` que o jogo já ship). Implementado `pvpKind: 'atk'|'def'` no node + `pvpSoulKind()` na soul (dados separam limpo: 8 souls PvP ofensivas AP/AR/crit vs 5 defensivas def/absorb/hp/evade/block) e a regra em `acceptsSoul` — aplicada em Planner (drag/swap), NodeEditor (lista), greedy, engine e filler. **Correção honesta**: a mudança anterior de `cv_5` pra PvP foi interpretação errada do 1º print — revertida pra Defesa (contagens voltam a 14 Def/4 PvP, como o jogo). Verificado no editor: node PvP-DEF lista só as 5 defensivas; PvP-ATK lista só as 8 ofensivas. Testes: 23.
11. **Baseline "nodes já abertas no jogo" (em teste na branch `dev`)** — nova opção no Gerador: "🔒 Partir das minhas nodes já abertas". O jogador marca na 🌳 Árvore o que JÁ abriu no jogo (nodes + níveis); com a opção ligada, o motor trata esses pontos como **custo afundado + piso** (`EngineConfig.baseline`): nodes ficam sempre abertos, custo sempre conta, nível nunca abaixa (reset no jogo exige item caro), e souls colocadas neles entram NO nível investido (valor de graça). Implementado em pathfinder (custo/open set), mutações, busca, filler (preenche os abertos primeiro), escadinha do greedy (orçamentos descontados + candidato "só aproveitar o aberto"), KnowledgeBase (baseline entra na chave do perfil) e consultor ("Respeitei suas N nodes…"). Só ativa com o checkbox marcado E nodes abertos; caso contrário, tudo como antes. 6 testes dedicados.
12. **Correção da topologia da árvore (barra de baixo PARTIDA)** — print do jogo provou que embaixo a esquerda (ataque) e a direita (defesa) NÃO se conectam à coluna do meio: as arestas h2_2—h2_3 e h2_3—h2_4 não existem no jogo e permitiam à IA "atravessar por baixo" (builds impossíveis). Removidas do `EDGE_PAIRS`; o meio de baixo (h2_3→bv3_8→bv3_9) só é alcançável pela espinha vertical. 4 testes de topologia travam isso.
13. **Tetos oficiais (nível 201 / 217 pontos) + correção de monotonicidade** — `MAX_FUSION_LEVEL=201`/`MAX_FUSION_POINTS=217` em `lib/formula.ts`, clampados em TODO lugar: input do nível (digitar 400 → 201), `totalFusionPoints`, store/normalize, `optimize()` (budget ∞ → 217), `deepOptimize()` e gerador rápido. **Bug real reportado**: build full-AP no nível 201 dava MENOS Attack Power que no 198 (greedy míope: 3 pontos a mais mudam decisões no meio e pioram o fim). Correção: **`bestGreedyGenome` (escadinha de orçamentos)** — roda o greedy em B, B−1, B−2, B−3, preenche cada um até o orçamento CHEIO com o fill pass e fica com o melhor score; usado no gerador rápido E como seed da busca profunda. Verificado no navegador (cenário do usuário AP100+Abs1): 198→201 = AP 737,16→**740,40** e AR +9,35, Absorb inalterado — os 3 pontos foram pro ataque. Testes: **27** (monotonicidade 214→217 com offense não-decrescente + tetos).
